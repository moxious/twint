...
def load_tweets_in_batches(filename):
    ...
    while True:
        try:
            ...
            print("Error on line %d of %s: %s" % (lineno, filename, e))
            break
...
LOAD_TWEETS_CYPHER = """
    ...
    WITH t
    RETURN count(t) as TweetsMerged;
"""

def load_tweets(driver, batch):
    def tweet_batch(tx):
        res = tx.run(LOAD_TWEETS_CYPHER, events=batch)
        rec = res.single()
        tm = rec.get("TweetsMerged")
        print("%d tweets merged" % tm)
        return tm
    
    with driver.session() as session:
        return session.write_transaction(tweet_batch)
...
def main(argv):
    ...
    for batch in batches:
        print("Writing batch %d of %d: %d tweets..." % (b, len(batches), len(batch)))
        tweets_merged = load_tweets(driver, batch)
        total_tweets = total_tweets + tweets_merged

    driver.close()
    print("Done!  A total of %d tweets merged" % total_tweets)
